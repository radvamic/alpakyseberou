...
export default function UkolyPage() {
  ...
  const draw = useCallback(() => {
    ...
    confetti({
      ...
      spread: 80,
      origin: { y: 0.5 },
      colors: ['#d8b28c', '#f1d5c4', '#c6706d', '#4d5a46', '#F5F0E8'],
    });
  }, [pool, level]);
    ...
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        confetti({
          ...
          spread: 120,
          origin: { y: 0.4 },
          colors: ['#d8b28c', '#f1d5c4', '#c6706d', '#4d5a46'],
        });
      } catch {
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] flex flex-col items-center px-4 py-10">
      ...
      <header className="w-full max-w-md flex items-start justify-between mb-8">
        <div>
          <h1
            className="font-[family-name:var(--font-great-vibes)] text-4xl text-[#d8b28c] leading-none"
          >
            {t.title}
          </h1>
        ...
        {(['mild', 'wild'] as const).map((lvl) => (
          <button
            ...
            className={`flex-1 py-2.5 rounded-full text-sm tracking-[0.15em] uppercase transition-all duration-300 ${
              level === lvl
                ? 'bg-[#d8b28c] text-[#0A0A0A] font-semibold shadow-lg shadow-[#d8b28c]/20'
            ...
            >
              <span
                className={`self-start text-[10px] tracking-[0.2em] uppercase font-semibold px-3 py-1 rounded-full mb-4 ${
                  current.level === 'mild'
                    ? 'bg-[#4d5a46]/30 text-[#9aaa92] border border-[#4d5a46]/40'
                    : 'bg-[#c6706d]/20 text-[#daa09d] border border-[#c6706d]/30'
                }`}
              >
      ...
      <button
        onClick={draw}
        className="w-full max-w-md mb-10 rounded-full border border-[#d8b28c] py-3.5 text-sm tracking-[0.18em] uppercase text-[#d8b28c] transition-all duration-300 hover:bg-[#d8b28c]/10 active:scale-[0.98]"
      >
      ...
      <section className="w-full max-w-md rounded-2xl border border-[#2A2520] bg-[#111111] p-6 mb-8">
        <h2 className="font-[family-name:var(--font-cormorant)] text-[#d8b28c] text-lg font-semibold tracking-[0.12em] uppercase mb-1">
          {t.proofTitle}
        </h2>
        ...
        <input
          ...
          disabled={uploading}
          className="w-full mb-3 rounded-xl border border-[#2A2520] bg-[#1A1A1A] px-4 py-2.5 text-sm text-[#F5F0E8] placeholder-[#3A3530] focus:border-[#d8b28c]/50 focus:outline-none transition-colors disabled:opacity-40"
        />
        ...
        <button
          ...
          disabled={uploading}
          aria-busy={uploading}
          className="w-full rounded-full border border-[#d8b28c]/60 py-3 text-sm tracking-[0.15em] uppercase text-[#d8b28c] transition-all duration-300 hover:bg-[#d8b28c]/10 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {uploading && (
            <span
              className="h-4 w-4 shrink-0 rounded-full border-2 border-[#d8b28c]/30 border-t-[#d8b28c] animate-spin"
              aria-hidden
            />
          )}
        ...
        <AnimatePresence>
          {uploading && (
            <motion.div
              ...
              aria-live="polite"
            >
              <div className="rounded-xl border border-[#d8b28c]/20 bg-[#d8b28c]/5 px-4 py-4 flex items-center gap-4">
                <span
                  className="h-10 w-10 shrink-0 rounded-full border-2 border-[#d8b28c]/25 border-t-[#d8b28c] animate-spin"
                  aria-hidden
                />
                <div className="text-left min-w-0">
                  <p className="text-sm text-[#d8b28c] font-medium tracking-wide">
                    {t.proofUploading}
                  </p>
