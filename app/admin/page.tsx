    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-10">
      ...
      <div className="flex items-center justify-between mb-10">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="text-xs tracking-[0.15em] uppercase text-[#7a6e65] hover:text-[#d8b28c] transition-colors disabled:opacity-40"
          >
            {emailTesting ? '⏳ Testuji…' : '✉ Test emailu'}
          </button>
          <button
            onClick={load}
            className="text-xs tracking-[0.15em] uppercase text-[#7a6e65] hover:text-[#d8b28c] transition-colors"
          >
            ↺ Obnovit
          </button>
          <button
      ...
      <div className="flex gap-1 mb-8 border-b border-[#2A2520]">
        {tabs.map((t) => (
          ...
          >
            {t.label}
            <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-sm ${tab === t.id ? 'bg-[#d8b28c]/20 text-[#d8b28c]' : 'bg-[#1a1a1a] text-[#5a5248]'}`}>
              {t.count}
            </span>
          </button>
        ))}
          ...
          {tab === 'rsvp' && (
            <div className="space-y-8">
              ...
              {Object.keys(menuCounts).length > 0 && (
                <div>
                  <h3 className="text-xs tracking-[0.2em] uppercase text-[#d8b28c] mb-3">Preference menu</h3>
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(menuCounts).map(([key, count]) => (
                      <span key={key} className="text-xs px-3 py-1.5 border border-[#2A2520] text-[#B8A99A]">
                        {key} <span className="text-[#d8b28c] ml-1">{count}×</span>
                      </span>
                    ))}
                  </div>
                </div>
              ...
              {Object.keys(stayCounts).length > 0 && (
                <div>
                  <h3 className="text-xs tracking-[0.2em] uppercase text-[#d8b28c] mb-3">Délka pobytu</h3>
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(stayCounts).map(([key, count]) => (
                      <span key={key} className="text-xs px-3 py-1.5 border border-[#2A2520] text-[#B8A99A]">
                        {STAY_LABELS[key] || key} <span className="text-[#d8b28c] ml-1">{count}×</span>
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex gap-2">
                {(['all', 'public', 'private'] as GuestbookFilter[]).map((f) => (
                  ...
                  >
                    ...
                    <span className={`ml-2 text-xs ${guestbookFilter === f ? 'text-[#d8b28c]' : 'text-[#5a5248]'}`}>
              ...
              {filteredGuestbook.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {filteredGuestbook.map((g) => (
                    <div key={g.id} className="relative border border-[#d8b28c]/10 p-6 hover:border-[#d8b28c]/25 transition-colors">
                      <span className="absolute top-0 left-0 w-3 h-3 border-t border-l border-[#d8b28c]/30" />
                      <span className="absolute top-0 right-0 w-3 h-3 border-t border-r border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-[#d8b28c]/30" />
                          ...
                          {g.photos.map((url, i) => (
                            <a key={i} href={url} target="_blank" rel="noreferrer">
                              ...
                              <img
                                src={url}
                                alt=""
                                className="w-16 h-16 object-cover border border-[#2A2520] hover:border-[#d8b28c]/50 transition-colors"
                              />
                            </a>
                          ))}
                        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 flex-wrap">
                <span className="text-xs px-3 py-1.5 border border-[#2A2520] text-[#B8A99A]">
                  Celkem <span className="text-[#d8b28c] ml-1">{photobooth.length}</span>
                </span>
                <span className="text-xs px-3 py-1.5 border border-[#2A2520] text-[#B8A99A]">
                  Veřejné <span className="text-[#d8b28c] ml-1">{photobooth.filter(p => p.isPublic).length}</span>
                </span>
                <span className="text-xs px-3 py-1.5 border border-[#2A2520] text-[#B8A99A]">
                  Soukromé <span className="text-[#d8b28c] ml-1">{photobooth.filter(p => !p.isPublic).length}</span>
                </span>
              </div>
              ...
              {photobooth.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {photobooth.map((p) => (
                    <div key={p.id} className="relative border border-[#d8b28c]/10 p-4 hover:border-[#d8b28c]/25 transition-colors">
                      <span className="absolute top-0 left-0 w-3 h-3 border-t border-l border-[#d8b28c]/30" />
                      <span className="absolute top-0 right-0 w-3 h-3 border-t border-r border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-[#d8b28c]/30" />
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <a href={p.originalPhotoUrl} target="_blank" rel="noreferrer">
                            ...
                            <img
                              ...
                              alt="originál"
                              className="w-8 h-8 object-cover border border-[#2A2520] hover:border-[#d8b28c]/50 transition-colors"
                              onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                            />
                          </a>
                        </div>
            ...
            <div className="space-y-8">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-6">
                    ...
                    <div className="text-center sm:text-left">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-block mt-3 text-xs text-[#d8b28c] underline underline-offset-2"
                      >
                        Otevřít stránku →
                      </a>
                    </div>
              ...
              {cameraSessions.length === 0 ? (
                ...
                <div className="space-y-3">
                  <h3 className="text-xs tracking-[0.2em] uppercase text-[#d8b28c]">Hosté</h3>
                  {cameraSessions.map((s) => (
                    <div key={s.id} className="relative border border-[#d8b28c]/10 hover:border-[#d8b28c]/25 transition-colors">
                      <span className="absolute top-0 left-0 w-3 h-3 border-t border-l border-[#d8b28c]/30" />
                      <span className="absolute top-0 right-0 w-3 h-3 border-t border-r border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 right-0 w-3 h-3 border-b border-r border-[#d8b28c]/30" />
                          ...
                          <div className="hidden sm:flex gap-[2px]">
                            {Array.from({ length: s.maxPhotos }).map((_, i) => (
                              <div
                                ...
                                style={{ backgroundColor: i < s.photosTaken ? '#d8b28c' : '#2A2520' }}
                              />
                            ))}
                          </div>
                              ...
                              <div key={photo.id} className="relative group">
                                <a href={photo.url} target="_blank" rel="noreferrer">
                                  ...
                                  <img
                                    ...
                                    alt=""
                                    className="w-full aspect-square object-cover border border-[#2A2520] hover:border-[#d8b28c]/50 transition-colors"
                                    onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                                  />
                                </a>
                                <button
              ...
              {guestPhotos.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {guestPhotos.map((photo) => (
                    <div
                      key={photo.id}
                      className="relative border border-[#d8b28c]/10 p-4 hover:border-[#d8b28c]/25 transition-colors"
                    >
                      <span className="absolute top-0 left-0 w-3 h-3 border-t border-l border-[#d8b28c]/30" />
                      <span className="absolute top-0 right-0 w-3 h-3 border-t border-r border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-[#d8b28c]/30" />
          ...
          {tab === 'table-challenge' && (
            <div className="space-y-8">
              ...
              {tableChallengeGuests.length === 0 ? (
                ...
                <div className="space-y-3">
                  <h3 className="text-xs tracking-[0.2em] uppercase text-[#d8b28c]">
                    Fotodůkazy podle hosta
                  </h3>
                  {tableChallengeGuests.map((g) => (
                    <div
                      key={g.name}
                      className="relative border border-[#d8b28c]/10 hover:border-[#d8b28c]/25 transition-colors"
                    >
                      <span className="absolute top-0 left-0 w-3 h-3 border-t border-l border-[#d8b28c]/30" />
                      <span className="absolute top-0 right-0 w-3 h-3 border-t border-r border-[#d8b28c]/30" />
                      <span className="absolute bottom-0 left-0 w-3 h-3 border-b border-l border-[#d8b28c]/30" />
                            ...
                            <div key={photo.id} className="flex gap-3 items-start border border-[#2A2520] p-2 rounded">
                              <a href={photo.url} target="_blank" rel="noreferrer" className="shrink-0">
                                ...
                                <img
                                  src={photo.thumbnailUrl || photo.url}
                                  alt=""
                                  className="w-16 h-16 object-cover border border-[#2A2520] hover:border-[#d8b28c]/50 transition-colors rounded"
                                />
                              </a>
                              <div className="flex-1 min-w-0">
                                {photo.challengeText && (
                                  <p className="text-xs text-[#d8b28c] mb-1 line-clamp-2 italic">„{photo.challengeText}"</p>
                                )}
              ...
              {tableChallengeGuests.length > 0 && (
                <div>
                  <h3 className="text-xs tracking-[0.2em] uppercase text-[#d8b28c] mb-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {tableChallengeGuests.flatMap((g) => g.photos).map((photo) => (
                      <div key={photo.id} className="relative border border-[#2A2520] p-2 space-y-1">
                        ...
                        {photo.challengeText && (
                          <p className="text-[10px] text-[#d8b28c] italic line-clamp-2">„{photo.challengeText}"</p>
                        )}
                        <p className="text-[10px] text-[#5a5248]">{formatDate(photo.createdAt)}</p>
                        <button
                          type="button"
