...
export async function sendRsvpNotification(data: {
  ...
  const html = `
    <div style="background:#0A0A0A;padding:40px 20px;font-family:Georgia,serif;max-width:560px;margin:0 auto">
      <div style="border-top:1px solid #C9A96E33;border-bottom:1px solid #C9A96E33;padding:32px;margin-bottom:24px">
        <p style="color:#C9A96E;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;margin:0 0 8px">Klára & Michal 2026</p>
      ...
      <p style="color:#5a5248;font-size:11px;margin-top:24px;text-align:center">
        Přehled všech odpovědí: <a href="${process.env.SITE_URL || 'https://alpakyseberou.cz'}/admin" style="color:#C9A96E">administrace</a>
      </p>
    </div>
  `;
...
export async function sendTestEmail(): Promise<{ ok: boolean; error?: string; config: Record<string, string> }> {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="background:#0A0A0A;padding:40px 20px;font-family:Georgia,serif;max-width:560px;margin:0 auto">
          <div style="border-top:1px solid #C9A96E33;border-bottom:1px solid #C9A96E33;padding:32px">
            <p style="color:#C9A96E;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;margin:0 0 8px">Klára & Michal 2026</p>
...
export async function sendGuestbookNotification(data: {
  ...
  const html = `
    <div style="background:#0A0A0A;padding:40px 20px;font-family:Georgia,serif;max-width:560px;margin:0 auto">
      ...
      <p style="color:#5a5248;font-size:11px;margin-top:24px;text-align:center">
        Přehled nástěnky: <a href="${process.env.SITE_URL || 'https://alpakyseberou.cz'}/admin" style="color:#C9A96E">administrace</a>
      </p>
    </div>
  `;
